...
const Navbar = () => {
  ...
  const { y: currentScrllY } = useWindowScroll(); // Using react-use to get the current scroll position
      ...
      <header className="absolute top-1/2 w-full -translate-y-1/2">
        <nav className="flex size-full items-center justify-between p-4">
          ...
          <div className="flex items-center gap-7">
            ...
            <a
              id="product-button"
              href="/sourav_mondal_react.pdf" // <- Place your PDF in `public/resume.pdf`
              target="_blank"
              rel="noopener noreferrer"
              className="bg-blue-50 flex items-center justify-center gap-3 px-7 py-3 rounded-full text-xs font-general uppercase text-black hover:bg-blue-100 transition"
            >
              <GiKatana />
              <span>RESUME</span>
              <SiKatana />
